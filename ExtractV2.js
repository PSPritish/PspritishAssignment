...
const MAX_RESULTS_PER_QUERY = 12;         // Based on observed API behavior

// Valid characters for query expansion: digits and lowercase letters
...
const PROGRESS_FILE = 'extracted_names_dynamicV2.json';
...
// Create a p-queue instance with initial concurrency and rate limiting: 50 requests per minute.
const queue = new PQueue({
    concurrency: currentConcurrency,
    intervalCap: 50,
    interval: 60000, // 60 seconds = 50 requests per minute
});
...
async function processPrefix(prefix) {
    ...
    if (results.length === MAX_RESULTS_PER_QUERY) {
        // Use the defined valid character set for expansion.
...
async function extractAllNames() {
    ...
    // Enqueue initial tasks for each character in the valid character set.
    for (const char of CHARSET) {
...
extractAllNames()
    .then(results => {
        ...
        saveToFile('names_only_dynamicV2.json', results.names);
    })
    .catch(err => {

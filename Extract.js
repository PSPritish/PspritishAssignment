...
const MAX_RESULTS_PER_QUERY = 10;         // Based on observed API behavior

// Use a given character set instead of ASCII range
...
const PROGRESS_FILE = 'extracted_names_dynamic.json';
...
// Create a p-queue instance with initial concurrency and rate limiting: 100 requests per minute.
const queue = new PQueue({
    concurrency: currentConcurrency,
    intervalCap: 100,
    interval: 60000, // 60 seconds = 100 requests per minute
});
...
async function queryAPI(prefix, attempt = 1) {
    ...
    try {
        ...
        if (error.response && error.response.status === 429) {
            console.log(`Rate limit encountered for prefix "${prefix}" on attempt ${attempt}. Waiting ${RATE_LIMIT_COOLDOWN_MS}ms...`);
            adjustConcurrency(Math.floor(currentConcurrency * CONCURRENCY_ADJUSTMENT_DOWN_FACTOR));
...
async function processPrefix(prefix) {
    ...
    if (results.length === MAX_RESULTS_PER_QUERY) {
        // Instead of ASCII range, iterate through the given character set.
    ...
    if (totalRequests % 100 === 0) {
        console.log(`Progress: ${totalRequests} requests, ${discoveredNames.size} unique names found.`);
    }
}

/**
 * Initialize the extraction process by enqueueing all single-letter prefixes.
 */
async function extractAllNames() {
    console.log('Starting dynamic concurrent extraction using defined character set...');
    const startTime = Date.now();

    // Start backlog monitoring.
    monitorBacklog();

    // Enqueue initial tasks for all letters in the given character set.
    for (const letter of CHARSET) {
        queue.add(() => processPrefix(letter));
    }
...
extractAllNames()
    .then(results => {
        ...
        saveToFile('names_only_dynamic.json', results.names);
    })
    .catch(err => {
